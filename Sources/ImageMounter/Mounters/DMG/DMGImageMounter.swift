...
public final class DMGImageMounter: ImageMounter, @unchecked Sendable {
    public let supportedTypes: [ImageType] = [.dmg, .sparseImage, .sparseBundle]
    ...
    public func mount(
        ...
        Logger.log(log, "Mounting DMG at \(url.path)", component: .dmgMounter)
        ...
        do {
            ...
            Logger.log(log, "hdiutil attach exit: \(attachResult.exitCode)", component: .dmgMounter)
            Logger.log(log, "stdout: \(attachResult.stdout)", component: .dmgMounter)
            Logger.log(log, "stderr: \(attachResult.stderr)", component: .dmgMounter)
            ...
            do {
                ...
                let rootDevices = Set(newDevices.map {
                    rootDevice(from: $0)
                })
            ...
            guard attachResult.exitCode == 0 else {
                try await cleanupDetach(using: parsed.info.deviceIdentifiers, log: log)
                throw MountError.mountFailed(reason: "hdiutil attach failed (exit \(attachResult.exitCode)): \(attachResult.stderr)")
            }

            Logger.log(log, "Parsed mount points: \(parsed.info.mountPoints)", component: .dmgMounter)
    ...
    public func unmount(_ result: MountResult, log: ImageMounterLogHandler? = nil) async throws {
        let hdiutil = SystemToolLocator.path(for: "hdiutil", log: log)
        let rootDevices = Set(result.deviceIdentifiers.map {
            rootDevice(from: $0)
        })
    ...
    private func cleanupDetach(using deviceIdentifiers: [String], log: ImageMounterLogHandler?) async throws {
        let hdiutil = SystemToolLocator.path(for: "hdiutil", log: log)
        let rootDevices = Set(deviceIdentifiers.map {
            rootDevice(from: $0)
        })
